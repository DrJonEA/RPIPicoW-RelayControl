   ...
   if( pxTaskStatusArray != NULL ){
	 ...
	 for( x = 0; x < uxArraySize; x++ )
	 {
      ...
      vPortFree( pxTaskStatusArray );
	   ...
	   printf("Fairelay to allocate space for stats\n");
   }
...
void main_task(void *params){
	...
	TLSTransBlock transport;

	mqttAgent.setObserver(&mqttObs);
	mqttAgent.setTransport(&transport);
	...
	TwinTask twinTask;
	twinTask.setStateObject(&relayState);
	twinTask.setMQTTInterface(&mqttAgent);
	twinTask.start(TASK_PRIORITY);

	RelayAgent relayAgent(RELAY_K1, SW2_GP, &mqttAgent, &relayState);
	relayAgent.start("RelayAgent", TASK_PRIORITY);

	MQTTRouterRelay router(&relayAgent);
	MQTTPingTask xPing;
	xPing.setInterface(&mqttAgent);
	xPing.start(TASK_PRIORITY);
	router.setPingTask(&xPing, &mqttAgent);
	router.setTwin(&twinTask);
